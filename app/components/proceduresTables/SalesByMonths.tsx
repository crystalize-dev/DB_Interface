import { SalesByMonth } from '@/app/types/ProcedureTypes';
import Link from 'next/link';
import React from 'react';

const SalesByMonths = ({ data }: { data: SalesByMonth[] }) => {
    const monthsMapping: { [key: string]: string } = {
        '1': 'Январь',
        '2': 'Февраль',
        '3': 'Март',
        '4': 'Апрель',
        '5': 'Май',
        '6': 'Июнь',
        '7': 'Июль',
        '8': 'Август',
        '9': 'Сентябрь',
        '10': 'Октябрь',
        '11': 'Ноябрь',
        '12': 'Декабрь'
    };

    return (
        <>
            <div className="flex w-full items-center bg-gradient-to-b from-dark-bg to-dark-object p-8 text-xl font-bold text-white">
                <h3 className="w-1/4">Год</h3>
                <h2 className="w-1/4">Месяц</h2>
                <h2 className="w-1/4">Заказов</h2>
                <h2 className="w-1/4">Доходность</h2>
            </div>

            <div className="scrollable flex w-full grow flex-col">
                {data.length > 0 ? (
                    data.map(
                        (row) =>
                            row && (
                                <Link
                                    href={`/customers/edit/${row.SalesMonth}`}
                                    key={String(row.SalesMonth)}
                                    className="relative flex w-full items-center border-b border-solid border-black/10 p-8 transition-all last:border-none hover:bg-white hover:shadow-sm"
                                >
                                    <p className="w-1/4">{row.SalesYear}</p>
                                    <p className="w-1/4">
                                        {monthsMapping[row.SalesMonth]}
                                    </p>
                                    <p className="w-1/4">{row.TotalOrders}</p>
                                    <p className="w-1/4">
                                        {row.TotalSalesAmount}
                                    </p>
                                </Link>
                            )
                    )
                ) : (
                    <div className="flex h-screen w-full items-center justify-center">
                        <p className="text-4xl text-zinc-500">Ничего нет!</p>
                    </div>
                )}
            </div>
        </>
    );
};

export default SalesByMonths;
